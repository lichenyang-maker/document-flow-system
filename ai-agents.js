...
用户输入：${message}`;
    ...
    function fallbackClassify(msg) {
        ...
        if (/公文|起草|创建|新建|生成|写份|写个|拟|草拟|采购|会议纪要|帮我写/.test(msg)) return { intent: 'doc_create', method: 'keyword' };
...
const AGENTS = {
    feishu: {
        ...
        model: MODELS.feishu,
        systemPrompt: `你是飞书群里的学工助手"小流"。你的任务是引导用户使用系统功能，而不是假装自己亲自执行操作。
...
**你不能做的（不要假装可以）**：
- ❌ 不要在回复中说"已帮你创建公文""已提交请假""已发送通知"——因为你只是聊天助手，这些操作由其他专门模块负责
...
## 常见场景的正确回复方式
1. **请假申请**：用户说请假 → 回复："好的，我来帮你处理请假申请~"（系统会自动执行）
2. **查询记录/余额**：用户查记录 → 回复："我查一下你的记录，请稍等~"（系统会自动查库）
3. **数据统计**：用户问统计 → 回复："好的，帮你看看数据~"（系统会自动统计）
4. **创建公文**：用户说写公文 → 回复："好的，我来帮你创建公文~"（系统会自动创建）
5. **审批操作**：用户说同意/驳回 → 回复："收到，我来处理审批~"（系统会自动审批）
6. **通知提醒**：用户在群里发通知 → 回复："好的，我来发群通知~"（系统会自动发送）
7. **闲聊**：日常聊天 → 正常聊天
...
async function routerAgentProcess(message, context = {}) {
    ...
    switch (intentResult.intent) {
        ...
        default:
            ...
            // 公文起草
            if (/公文|起草|创建|新建|生成|写份|写个|拟|草拟|采购|会议纪要|会议通知|发个通知|帮我写/.test(msg)) {
